/**
 * Creating a sidebar enables you to:
 - create an ordered group of docs
 - render a sidebar for each doc of that group
 - provide next/previous navigation

 The sidebars can be generated from the filesystem, or explicitly defined here.

 Create as many sidebars as you want.
 */

// @ts-check

/** @type {import('@docusaurus/plugin-content-docs').SidebarsConfig} */
const sidebars = {
  tutorialSidebar: [
    // dataverse-simulate
    {
      type: 'category',
      label: 'dataverse-simulate',
      link: {
        type: "doc",
        id: 'dataverse-simulate/index'
      },
      items: [
        'dataverse-simulate/mocking-for-the-organization-service',
        'dataverse-simulate/mocking-for-plugins',
        // simulator-options
        {
          type: 'category',
          label: 'Simulator options',
          link: {
            type: "doc",
            id: 'dataverse-simulate/simulator-options/simulator-options'
          },
          items: [
            'dataverse-simulate/simulator-options/initialise-data',
            'dataverse-simulate/simulator-options/setting-authenticated-user',
            'dataverse-simulate/simulator-options/configure-user-business-unit',
            'dataverse-simulate/simulator-options/setting-plugin-context',
            'dataverse-simulate/simulator-options/setting-system-time',
            'dataverse-simulate/simulator-options/setting-entity-processors',
            'dataverse-simulate/simulator-options/configure-fiscal-year-settings',
            'dataverse-simulate/simulator-options/configure-organization',
            'dataverse-simulate/simulator-options/faking-service-failures',
            'dataverse-simulate/simulator-options/simulate-security-model'
          ]
        },
        // data-stores
        {
          type: 'category',
          label: 'Data stores',
          link: {
            type: "doc",
            id: 'dataverse-simulate/data-stores/data-stores'
          },
          items: [
            {
              type: 'category',
              label: 'Entity data',
              link: {
                type: "doc",
                id: 'dataverse-simulate/data-stores/entity-data/index'
              },
              items: [
                {
                  type: 'autogenerated',
                  dirName: 'dataverse-simulate/data-stores/entity-data',
                }
              ]
            },
            {
              type: 'category',
              label: 'Plugin traces',
              link: {
                type: "doc",
                id: 'dataverse-simulate/data-stores/plugin-traces/index'
              },
              items: [
                {
                  type: 'autogenerated',
                  dirName: 'dataverse-simulate/data-stores/plugin-traces',
                }
              ]
            },
            {
              type: 'category',
              label: 'Application telemetry',
              link: {
                type: "doc",
                id: 'dataverse-simulate/data-stores/application-telemetry/index'
              },
              items: [
                {
                  type: 'autogenerated',
                  dirName: 'dataverse-simulate/data-stores/application-telemetry',
                }
              ]
            },
            {
              type: 'category',
              label: 'Simulator audits',
              link: {
                type: "doc",
                id: 'dataverse-simulate/data-stores/simulator-audits/index'
              },
              items: [
                {
                  type: 'autogenerated',
                  dirName: 'dataverse-simulate/data-stores/simulator-audits',
                }
              ]
            },
          ]
        },
        // entity-processors
        {
          type: 'category',
          label: 'Entity processors',
          link: {
            type: "doc",
            id: 'dataverse-simulate/entity-processors/entity-processors'
          },
          items: [
            'dataverse-simulate/entity-processors/create',
            'dataverse-simulate/entity-processors/update'
          ],
        },
        'dataverse-simulate/supported-sdk-messages',
        'dataverse-simulate/inject-custom-organization-requests',
        // release-notes
        {
          type: 'category',
          label: 'Release notes',
          items: [
            {
              type: 'autogenerated',
              dirName: 'dataverse-simulate/release-notes',
            }
          ],
        }
      ],
    },
    // markdown-maker
    {
      type: 'category',
      label: 'markdown-maker',
      link: {
        type: "doc",
        id: 'markdown-maker/index'
      },
      items: [
        "markdown-maker/document-and-document-sets",
        "markdown-maker/content-types",
        {
          type: 'category',
          label: 'Document Parts',
          items: [
            {
              type: 'autogenerated',
              dirName: 'markdown-maker/document-parts'
            }
          ],
        },
        {
          type: 'category',
          label: 'Platform Extensions',
          link: {
            type: "doc",
            id: 'markdown-maker/platform-extensions/index'
          },
          items: [
            {
              type: 'autogenerated',
              dirName: 'markdown-maker/platform-extensions'
            }
          ],
        }
      ]
    },
    // build-monitor
    {
      type: 'category',
      label: 'build-monitor',
      link: {
        type: "doc",
        id: 'react-build-monitor/index'
      },
      items: [
        "react-build-monitor/dashboard-data-model",
        "react-build-monitor/custom-styling",
        "react-build-monitor/sample-data-retrieval-functions",
      ]
    },
    // cds-customisation
    {
      type: 'category',
      label: 'cds-customisation',
      link: {
        type: "doc",
        id: 'cds-customisation/index'
      },
      items: [
        "cds-customisation/plugin-registration",
        "cds-customisation/manifest-organisation-patterns",
        "cds-customisation/generate-customisation",
        "cds-customisation/migrate-bulk-deletion-jobs",
        "cds-customisation/toggle-process-activation",
        "cds-customisation/documentation-generator",
        "cds-customisation/portal-generation",
      ]
    },
    // portal-gds-accelerator
    {
      type: 'category',
      label: 'portal-gds-accelerator',
      link: {
        type: "doc",
        id: 'portal-gds-accelerator/index'
      },
      items: [
        {
          type: 'autogenerated',
          dirName: 'portal-gds-accelerator',
        }
      ]
    },
    // portal-test-framework
    {
      type: 'category',
      label: 'portal-test-framework',
      link: {
        type: "doc",
        id: 'portal-test-framework/index'
      },
      items: [
        {
          type: 'autogenerated',
          dirName: 'portal-test-framework',
        }
      ]
    },
    // gds-wireframe-stencils
    {
      type: 'category',
      label: 'gds-wireframe-stencils',
      link: {
        type: "doc",
        id: 'gds-wireframe-stencils/index'
      },
      items: [
        {
          type: 'autogenerated',
          dirName: 'gds-wireframe-stencils',
        }
      ]
    },
    // break
    {
      type: "html",
      value: "<hr/>"
    },
    // benefits-architecture-framework
    {
      type: 'category',
      label: 'Benefits architecture',
      link: {
        type: "doc",
        id: 'benefits-architecture/index'
      },
      items: [
          "benefits-architecture/index",
          "benefits-architecture/principles",
          "benefits-architecture/operating-model",
          "benefits-architecture/roles-and-accountabilities",
          "benefits-architecture/scrum-integration",
          "benefits-architecture/framework-alignment",
          "benefits-architecture/engagement-playbook",
          "benefits-architecture/consulting-offering",
          "benefits-architecture/maturity-model",
          "benefits-architecture/glossary",
          "benefits-architecture/references",
        {
          type: 'category',
          label: 'Artefacts',
          items: [
            {
              type: 'autogenerated',
              dirName: 'benefits-architecture/artefacts',
            }
          ]
        },
        {
          type: 'category',
          label: 'Examples',
          items: [
            {
              type: 'autogenerated',
              dirName: 'benefits-architecture/examples',
            }
          ]
        }
      ]
    },
    // style-guide
    {
      type: 'category',
      label: 'Style guide',
      link: {
        type: "doc",
        id: 'style-guide/index'
      },
      items: [
        // Repos and code standards
        {
          type: 'category',
          label: 'Repo and code standards',
          link: {
            type: "doc",
            id: 'style-guide/repos/intro'
          },
          items: [
            {
              type: 'autogenerated',
              dirName: 'style-guide/repos',
            }
          ]
        },
        // Agile and devops
        {
          type: 'category',
          label: 'Agile and devops',
          link: {
            type: "doc",
            id: 'style-guide/agile-and-devops/intro'
          },
          items: [
            {
              type: 'autogenerated',
              dirName: 'style-guide/agile-and-devops',
            }
          ]
        },
        // Platforms
        {
          type: 'category',
          label: 'Platform best practices',
          link: {
            type: "doc",
            id: 'style-guide/platforms/intro'
          },
          items: [
            {
              type: 'autogenerated',
              dirName: 'style-guide/platforms',
            }
          ]
        },
      ]
    },
    // -- Personal Projects --
    //
    // break
    {
      type: "html",
      value: "<hr/>"
    },
    // obsidian folk tune review CLI
    {
      type: 'category',
      label: 'obsidian-folk-tune-review',
      className: 'sidebar-item--personal',
      link: {
        type: "doc",
        id: 'personal/obsidian-folk-tune-review/index'
      },
      items: [
        "personal/obsidian-folk-tune-review/selection-logic",
        "personal/obsidian-folk-tune-review/review-command",
        "personal/obsidian-folk-tune-review/pick-command",
        "personal/obsidian-folk-tune-review/session-command",
        "personal/obsidian-folk-tune-review/stats-command",
        "personal/obsidian-folk-tune-review/admin-commands"
      ]
    },
  ],
};

{
  module.exports = sidebars;
}
